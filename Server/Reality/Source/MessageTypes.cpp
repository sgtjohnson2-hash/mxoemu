...
PlayerDetailsMsg::PlayerDetailsMsg( PlayerObject *thePlayer )
{
	...
	m_buf.writeString(thePlayer->getCrewName());
	size_t factionNameStrLenPos = m_buf.wpos();
	m_buf.writeString(thePlayer->getFactionName());
...
HealthUpdateMsg::HealthUpdateMsg( uint32 objectId, bool includeMax, bool includeDead )
	:ObjectUpdateMsg(objectId),m_includeMax(includeMax),m_includeDead(includeDead)
{
}
...
const ByteBuffer& HealthUpdateMsg::toBuf()
{
	...
	if (m_includeDead)
		block.addByte(PlayerAttrOther::IsDead,m_player->isDead() ? 1 : 0);
...
CombatHitFxMsg::CombatHitFxMsg( uint32 objectId, uint32 fxId, uint8 hitCounter )
	:ObjectUpdateMsg(objectId),m_fxId(fxId),m_hitCounter(hitCounter)
{
}

CombatHitFxMsg::~CombatHitFxMsg()
{
}

const ByteBuffer& CombatHitFxMsg::toBuf()
{
	m_buf.clear();
...
CombatantModeMsg::CombatantModeMsg( uint32 objectId, uint8 mode )
	:ObjectUpdateMsg(objectId),m_mode(mode)
{
}

CombatantModeMsg::~CombatantModeMsg()
{
}

const ByteBuffer& CombatantModeMsg::toBuf()
{
	m_buf.clear();
	...
	AttributeUpdateBlock block;
	block.addByte(PlayerAttrOther::CombatantMode,m_mode);
...
SelfVitalsMsg::SelfVitalsMsg( PlayerObject *thePlayer, bool includeMax, bool includeDead )
{
	m_buf.clear();
	if (thePlayer == NULL)
		throw PacketNoLongerValid();
...
SelfCombatantModeMsg::SelfCombatantModeMsg( uint8 mode )
{
	...
	AttributeUpdateBlock block;
	block.addByte(PlayerAttrSelf::CombatantMode,mode);
...
SpawnILCombatHandlerMsg::SpawnILCombatHandlerMsg( uint16 viewId, uint8 spawnIdCounter, LocationVector pos, float simTime )
{
	...
	//03 [view 01 00 = object manager] [creation packet] [viewId] 00
	m_buf << uint8(0x03);
	m_buf << uint16(VIEWID_OBJECTMANAGER);

	//creation packet: 0C (fully dynamic) [goid:2] [idCounter] CD AB [attributes]
	m_buf << uint8(0x0C);
	m_buf << uint16(GOID_ILCOMBATHANDLER);
	m_buf << uint8(spawnIdCounter);
	m_buf << uint8(0xCD);
	m_buf << uint8(0xAB);

	//creation attributes: count=2, group header bits 0(Position)+2(StartTime)=0x05
	m_buf << uint8(0x02);
	m_buf << uint8(0x05);
