...
// ---------------------------------------------------------------------------
// combat / view attribute update messages
// wire formats recovered from the CR2 client and HDS captures
// ---------------------------------------------------------------------------

//attribute update carrying current health (and optionally max/death state)
//for a player's view as other clients see it
class HealthUpdateMsg : public ObjectUpdateMsg
{
public:
	HealthUpdateMsg(uint32 objectId, bool includeMax=false, bool includeDead=false);
	~HealthUpdateMsg();
	const ByteBuffer& toBuf();
private:
	bool m_includeMax;
...
//combat hit as other clients see it: health change + hit FX on the victim's view
class CombatHitFxMsg : public ObjectUpdateMsg
{
public:
	CombatHitFxMsg(uint32 objectId, uint32 fxId, uint8 hitCounter);
	~CombatHitFxMsg();
	const ByteBuffer& toBuf();
private:
	uint32 m_fxId;
...
//combatant mode (combat stance / interlock state) on another player's view
class CombatantModeMsg : public ObjectUpdateMsg
{
public:
	CombatantModeMsg(uint32 objectId, uint8 mode);
	~CombatantModeMsg();
	const ByteBuffer& toBuf();
private:
	uint8 m_mode;
...
//self-view (view 2) vitals update: health + inner strength bars, optionally
//max values and death state - this is what drives the client's own HUD bars
class SelfVitalsMsg : public StaticMsg
{
public:
	SelfVitalsMsg(class PlayerObject *thePlayer, bool includeMax=false, bool includeDead=false);
	~SelfVitalsMsg() {}
};

//self-view combat hit: own health + hit FX (what the victim's client shows)
class SelfHitFxMsg : public StaticMsg
{
public:
	SelfHitFxMsg(class PlayerObject *thePlayer, uint32 fxId, uint8 hitCounter);
	~SelfHitFxMsg() {}
};

//self-view combatant mode change
class SelfCombatantModeMsg : public StaticMsg
{
public:
	SelfCombatantModeMsg(uint8 mode);
...
{
public:
	CastBarMsg(uint16 abilityId, float castTime)
	{
		m_buf.clear();
		m_buf << uint16(swap16(0x80ac));
		m_buf << uint16(abilityId);
		for (int i=0;i<10;i++)
			m_buf << uint8(0);
		m_buf << float(castTime);
	}
	~CastBarMsg() {}
};

//ability loaded into memory response (RPC 0x80b2)
class AbilityLoadRspMsg : public StaticMsg
{
public:
	AbilityLoadRspMsg(uint16 abilityId, uint16 level, uint16 slot)
	{
		m_buf.clear();
		m_buf << uint16(swap16(0x80b2));
		m_buf << uint16(abilityId);
		m_buf << uint16(level);
		m_buf << uint16(slot);
	}
...
//ability unloaded from memory response (RPC 0x80b3)
class AbilityUnloadRspMsg : public StaticMsg
{
public:
	AbilityUnloadRspMsg(uint16 abilityId)
	{
		m_buf.clear();
		m_buf << uint16(swap16(0x80b3));
		m_buf << uint16(abilityId);
	}
	~AbilityUnloadRspMsg() {}
};
...
{
public:
	DeleteViewMsg(uint16 viewId)
	{
		m_buf.clear();
		const byte rawData[6] = { 0x03, 0x01, 0x00, 0x01, 0x01, 0x00 };
...
//spawns an ILCombatHandler game object view (GOID 55) - the client-side
//component that drives the interlock camera and round UI
class SpawnILCombatHandlerMsg : public StaticMsg
{
public:
	SpawnILCombatHandlerMsg(uint16 viewId, uint8 spawnIdCounter, class LocationVector pos, float simTime);
	~SpawnILCombatHandlerMsg() {}
};

//interlock pairing setup, sent right after the IL handler view spawns
//(structure from live CR2 combat captures - partially understood)
