...
#pragma pack(pop)
	...
	//scope for db ptr
	{
		scoped_ptr<QueryResult> result(sDatabase.Query(format("SELECT `userId`, `username`, `passwordSalt`, `passwordHash`, `publicExponent`, `publicModulus`, `privateExponent`, `timeCreated` FROM `users` WHERE LOWER(`username`) = LOWER('%1%') LIMIT 1") % sDatabase.EscapeString(m_username) ) );
		if (result == NULL)
		{
			// Check if username starts with "register "
			if (m_username.length() > 9 && _strnicmp(m_username.c_str(), "register ", 9) == 0)
			{
				m_isNewUser = true;
				m_username = m_username.substr(9); // Strip "register "
...
void AuthSocket::HandleAuthChallengeResponse( ByteBuffer &packet )
{
	...
	if (m_isNewUser)
	{
		string plaintextPass(&password[0]);

		// Generate a new user in the database
		sAuth.CreateAccount(m_username, plaintextPass);

		INFO_LOG(format("Successfully registered new user %1%, disconnecting them so they can log in normally.") % m_username);
		
		// Wait, if we just disconnect them, they get a generic error.
		// That's fine for now, they can just log in normally afterwards.
		SetCloseAndDelete(true);
		return;
	}
