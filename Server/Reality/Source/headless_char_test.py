...
def run_test(host: str, auth_port: int, margin_port: int, username: str, password: str, handle_prefix: str) -> bool:
    ...
    try:
        ...
        opcode, auth_result, unk1, user_id, off_auth_data, off_enc_data, unk2, unk3, off_world, off_char, off_user = struct.unpack_from("<BIHIHHHHHHH", auth_reply, 0)
        ...
        enc_len = struct.unpack_from("<H", auth_reply, off_enc_data)[0]
        enc_bytes = auth_reply[off_enc_data + 2 : off_enc_data + 2 + enc_len]

        dec_auth = tf_decrypt_cbc(enc_bytes, auth_twofish_key)
        user_d = int.from_bytes(dec_auth[:96], 'big')
        user_modulus = int.from_bytes(ticket_bytes[210 : 210 + 96], 'big')
