...
static inline bool SafeWriteFloat(void* ptr, float val) {
    if (!ptr || (uintptr_t)ptr < 0x10000 || (uintptr_t)ptr >= 0x7FFE0000) return false;
    __try {
        MEMORY_BASIC_INFORMATION mbi;
...
static volatile bool      s_charCreationSubmitted = false;
static volatile int       s_charCreationTicks = 0;

static volatile int       s_state4Ticks = 0;       // WorldMgr State 4 Streaming ticks
static volatile int       s_marginState4Ticks = 0; // Margin State 4 selection ticks

static volatile bool      s_screen4BActive = false;
...
static int GetMarginStateId() {
    ...
    void* pStateObj = *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x10);
    if (!pStateObj) return -1;
    uintptr_t* pVtbl = *reinterpret_cast<uintptr_t**>(pStateObj);
    if (!pVtbl) return -1;
    typedef int (__thiscall *GetStateId_t)(void* pThis);
    GetStateId_t pfnGetStateId = reinterpret_cast<GetStateId_t>(pVtbl[6]);
...
static void ParseClientCommandLine() {
    ...
    if (strstr(cmd, "-autojackin") || strstr(cmd, "/autojackin")) {
...
int __fastcall DetourGetCharacterCount(void* pThis, void* /*edx*/) {
    ...
    // 1. Check if real characters were received from MarginServer:
    ...
    // 2. If auto-jackin or command-line character requested, mount operative and return 1
...
// 0x00429F20: MarginMgr::SelectCharacter(uint32_t charIndex)
...
static void __fastcall DetourMarginSelectChar(void* pMgr, void* /*edx*/, uint32_t charIndex) {
...
// s_state4Ticks, s_screen5DActive, s_screen5DFrames declared above
...
static inline void SafeHideControl(uintptr_t clientBase, void* pUI, DWORD ctrlId) {
    if (clientBase && pUI) {
        typedef void (__thiscall *HideControl_t)(void* pUI, DWORD id);
        HideControl_t pHide = reinterpret_cast<HideControl_t>(clientBase + 0x0001D3C0);
        __try {
            pHide(pUI, ctrlId);
        } __except (EXCEPTION_EXECUTE_HANDLER) {}
    }
...
// Hook client.dll + 0x0002AC10 (Play selected character from Screen 0x4B)
...
static char s_playHandles[5][64];
static MxoPlayVectorEntry s_playEntries[5];

static void __fastcall DetourClientPlayChar(void* pThis, void* /*edx*/) {
    ...
    if (pMarginMgr) {
        ...
        Log("[mxohax] DetourClientPlayChar: Calling MarginMgr::SelectCharacter(0x00429F20) with index %d...\n", selectedIndex);
    ...
    if (g_clientBase) {
        // Ensure vector at clientBase + 0x00899B4C is valid up to selectedIndex
        DWORD* ppCharBegin = reinterpret_cast<DWORD*>(g_clientBase + 0x00899B4C);
        DWORD* ppCharEnd   = reinterpret_cast<DWORD*>(g_clientBase + 0x00899B50);
        int curVecCount = (ppCharBegin && ppCharEnd && *ppCharEnd >= *ppCharBegin) ? (*ppCharEnd - *ppCharBegin) / 32 : 0;
        ...
        for (int i = 0; i <= selectedIndex && i < 5; ++i) {
            ...
            const char* hName = g_ActiveCharName;
            uint32_t cId = g_ActiveCharId;
            if (pMarginMgr) {
                uintptr_t pE = *reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x644 + i * 4);
                if (pE && pE != reinterpret_cast<uintptr_t>(&g_SyntheticCharObj)) {
                    uintptr_t pData = *reinterpret_cast<uintptr_t*>(pE + 0x14);
                    if (pData) {
                        const char* name = reinterpret_cast<const char*>(pData + 3);
                        if (name && name[0] && strlen(name) < 32) hName = name;
                    }
                    uintptr_t pRec = *reinterpret_cast<uintptr_t*>(pE + 0x10);
                    if (pRec) {
                        uint32_t id = *reinterpret_cast<uint32_t*>(pRec + 3);
                        if (id > 0) cId = id;
                    }
                }
            }
            strncpy_s(s_playHandles[i], sizeof(s_playHandles[i]), hName, _TRUNCATE);
            s_playEntries[i].pHandleFirst = s_playHandles[i];
            s_playEntries[i].pHandleLast  = s_playHandles[i] + strlen(s_playHandles[i]);
            s_playEntries[i].pHandleEnd   = s_playEntries[i].pHandleLast;
            s_playEntries[i].charId       = cId;
            s_playEntries[i].worldId      = 1;
        }

        if (ppCharBegin && ppCharEnd && curVecCount <= selectedIndex) {
            *ppCharBegin = reinterpret_cast<DWORD>(&s_playEntries[0]);
            *ppCharEnd   = reinterpret_cast<DWORD>(&s_playEntries[selectedIndex + 1]);
            Log("[mxohax] DetourClientPlayChar: Mounted %d operative entries into 0x00899B4C\n", selectedIndex + 1);
        }

        *reinterpret_cast<const char**>(g_clientBase + 0x00896E4C) = s_playWorldPath;
        *reinterpret_cast<BYTE*>(g_clientBase + 0x0089DD5D) = 1;
        ...
        void* pUI = GetCUIPointer(g_clientBase);
        if (pUI) {
            SafeSetControlVisible(g_clientBase, pUI, 0x4B, false);
            SafeHideControl(g_clientBase, pUI, 0x4B);
        }
    }
    s_screen4BActive = false;

    if (OriginalClientPlayChar) {
        Log("[mxohax] DetourClientPlayChar: Calling original 0x1002AC10 to invoke EnterWorldWithCharacter...\n");
    ...
    if (g_clientBase) {
        void* pWorldMgr = *reinterpret_cast<void**>(g_clientBase + 0x0089DD68);
        if (pWorldMgr) {
            DWORD curState = *reinterpret_cast<DWORD*>(reinterpret_cast<DWORD>(pWorldMgr) + 0x1C);
            Log("[mxohax] DetourClientPlayChar: After OriginalClientPlayChar, WorldMgr state is %u\n", curState);
            if (curState < 2) {
                *reinterpret_cast<BYTE*>(g_clientBase + 0x0089DD5D) = 1;
                *reinterpret_cast<BYTE*>(reinterpret_cast<DWORD>(pWorldMgr) + 0x25) = 1;
                *reinterpret_cast<DWORD*>(reinterpret_cast<DWORD>(pWorldMgr) + 0x1C) = 0;
                typedef char (__thiscall *AdvanceState_t)(void* pMgr);
                AdvanceState_t pAdvance = reinterpret_cast<AdvanceState_t>(g_clientBase + 0x00120060);
                char advRes = pAdvance(pWorldMgr);
                Log("[mxohax] DetourClientPlayChar: AdvanceState(0x10120060) returned %d! pWorldMgr State is now %u\n",
                    advRes, *reinterpret_cast<DWORD*>(reinterpret_cast<DWORD>(pWorldMgr) + 0x1C));

                Log("[mxohax] DetourClientPlayChar: Direct fallback calling EnterWorldWithCharacter...\n");
                typedef void (__thiscall *EnterWorld_t)(void* pMgr, void* pChar);
                EnterWorld_t pEnterWorld = reinterpret_cast<EnterWorld_t>(g_clientBase + 0x00124070);
                __try {
                    pEnterWorld(pWorldMgr, &s_playEntries[selectedIndex]);
                } __except (EXCEPTION_EXECUTE_HANDLER) {
                    Log("[mxohax] DetourClientPlayChar: Exception caught in EnterWorldWithCharacter\n");
                }
            }
...
// Hook ViewPlayerSelection::SelectCharacterInList (client.dll + 0x000C2120)
...
static void __fastcall DetourSelectCharacterInList(void* pThis, void* /*edx*/) {
...
// Hook CLARSICharCreateView::HandleMessage (client.dll + 0x0004AE30)
...
static int __fastcall DetourCharCreateHandleMessage(void* pThis, void* /*edx*/, void* pMsg) {
    if (pMsg) {
...
// Hook for client.dll SGI STL pool allocator refill (client.dll + 0x00001B50)
typedef void* (__cdecl *Refill_t)(size_t n);
static Refill_t OriginalRefill = nullptr;

static void* __cdecl DetourRefill(size_t n) {
    if (n == 0) n = 8;
    size_t rounded = (n + 7) & ~7;
    int nobjs = 64;

    // Allocate real memory pool slab directly using standard malloc
    ...
    if (clientBase && rounded <= 128) {
        ...
        *reinterpret_cast<void**>(curObj) = nullptr;
        ...
        static int s_refillLogCount = 0;
        if (s_refillLogCount < 5) {
            Log("[mxohax] DetourRefill: Allocated %d blocks of size %u at 0x%p for freelist #%u\n",
                nobjs, rounded, chunk, idx);
            s_refillLogCount++;
        }
        return result;
    }
...
// Hook for client.dll SGI STL pool allocator deallocate (client.dll + 0x00001BC0)
typedef void (__cdecl *PoolFree_t)(void* p, size_t n);
static PoolFree_t OriginalPoolFree = nullptr;

static void __cdecl DetourPoolFree(void* p, size_t n) {
    if (!p) return; // Prevent free(NULL) crash at 0x10001C1C

    HMODULE hSelf = GetModuleHandleA("mxohax_modern.dll");
    uintptr_t selfBase = (uintptr_t)hSelf;
    if (selfBase && (uintptr_t)p >= selfBase && (uintptr_t)p < selfBase + 0x200000) {
        return; // Guard static mxohax strings from being freed into pool!
    }
    ...
    if (OriginalPoolFree) {
        __try {
            ...
            uintptr_t clientBase = GetSafeClientBase();
            if (clientBase) {
                *reinterpret_cast<DWORD*>(clientBase + 0x00896C10) = 0; // Release pool lock if crashed
            }
        }
    }
}
...
static bool TryAutoJackIn(DWORD clientBase) {
    ...
    // 1. Phase 1: Show authentic 2D Loading Screen (0x57) and dismiss Login (0x30), Char Select (0x4B), and Char Create (0x5D)
    ...
    // 3. Mark character selected in client.dll WorldMgr and advance to State 2
    ...
    *reinterpret_cast<BYTE*>(reinterpret_cast<DWORD>(pWorldMgr) + 0x25) = 1;
    ...
    // 4. Transition matrix.exe Margin State Machine
    void* pMarginMgr = *reinterpret_cast<void**>(0x004B3A44);
    if (pMarginMgr) {
        ...
        if (curMarginState == 4) {
            typedef void (__thiscall *MarginSelectChar_t)(void* pMgr, uint32_t charIndex);
            MarginSelectChar_t pfnSelect = reinterpret_cast<MarginSelectChar_t>(0x00429F20);
            Log("[mxohax] [AutoJackIn] Margin State 4: Invoking SelectCharacter(0x00429F20)...\n");
        ...
        if (curMarginState != 9) {
            ...
            Log("[mxohax] [AutoJackIn] Invoking matrix.exe Margin State transition from State %d to State 9 (0x00428FF0)...\n", curMarginState);
            Transition(pMarginMgr, 9);
        }
        *reinterpret_cast<BYTE*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x778) = 1;
        *reinterpret_cast<DWORD*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x77c) = 1;
        Log("[mxohax] [AutoJackIn] Margin state transition handling complete (curState=%d)!\n", GetMarginStateId());
    }
    ...
    *reinterpret_cast<const char**>(clientBase + 0x00896E4C) = s_localCharEntry.pWorldFirst;
    ...
    if (ppCharBegin && ppCharEnd) {
        ...
        *ppCharEnd   = reinterpret_cast<DWORD>(&s_localCharEntry) + sizeof(s_localCharEntry);
        Log("[mxohax] [AutoJackIn] Mounted operative %s (slums_barrens_full.metr, charId=%u) into vector at 0x00899B4C\n",
            s_localCharEntry.pHandleFirst ? s_localCharEntry.pHandleFirst : "Slacker", s_localCharEntry.charId);
    }
    ...
    if (pCharToEnter) {
        ...
        __try {
            ...
            Log("[mxohax] [AutoJackIn] Exception in EnterWorldWithCharacter caught safely!\n");
        }
        DWORD* pCurState = reinterpret_cast<DWORD*>(reinterpret_cast<DWORD>(pWorldMgr) + 0x1C);
        if (pCurState && *pCurState < 4) {
            *pCurState = 4; // Advance to State 4 (Streaming)
        }
    ...
    __try {
        ...
        if (pLayoutMgr && !IsBadReadPtr(reinterpret_cast<void*>(pLayoutMgr), 0x30)) {
            ...
            if (pLayoutRoot && IsValidWidget(clientBase, pLayoutRoot)) {
                ...
                *reinterpret_cast<DWORD*>(reinterpret_cast<uintptr_t>(pLayoutRoot) + 0x28) &= ~0x00000001;
                *reinterpret_cast<DWORD*>(reinterpret_cast<uintptr_t>(pLayoutRoot) + 0x28) |= 0x00000010;
            }
        }
        ...
        if (pRoot24 && IsValidWidget(clientBase, pRoot24)) {
            ...
            *reinterpret_cast<DWORD*>(reinterpret_cast<uintptr_t>(pRoot24) + 0x28) &= ~0x00000001;
            *reinterpret_cast<DWORD*>(reinterpret_cast<uintptr_t>(pRoot24) + 0x28) |= 0x00000010;
        }
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            // Empty slots have no static icon texture assigned, which previously rendered as flat solid white squares.
            // Clear flags and zero size so the slot shows the authentic textured background housing (pToolbarImg).
            ...
            if (pIconStatic && IsValidWidget(clientBase, pIconStatic)) {
                ...
                *reinterpret_cast<int*>(reinterpret_cast<uintptr_t>(pIconStatic) + 0x74) = 0;
                *reinterpret_cast<int*>(reinterpret_cast<uintptr_t>(pIconStatic) + 0x78) = 0;
            }

            // Slot active ability icon (+0x04: Image_Icon)
            // Empty slots have no active ability texture assigned.
            // Clear flags and zero size to prevent flat solid white quad fills and eliminate invalid offset dereferences.
            ...
            if (pIcon && IsValidWidget(clientBase, pIcon)) {
                ...
                *reinterpret_cast<int*>(reinterpret_cast<uintptr_t>(pIcon) + 0x74) = 0;
                *reinterpret_cast<int*>(reinterpret_cast<uintptr_t>(pIcon) + 0x78) = 0;
            }
...
static void RepositionCompass(uintptr_t clientBase, void* pUI, int screenW, int screenH) {
    ...
    // Left wing: Cyan Operative (+0x94, +0x98, +0x9C, w=42, h=30)
    // Symmetrical to Right Wing (center 960 - 80 - 42 = 838 at 1080p)
    const int targetLeftX = (screenW / 2) - 80 - 42; // 838 at 1920p
    const int targetLeftY = targetBaseY + 99;         // 1045 at 1080p

    // Right wing: Cell Phone / Mission (+0x8C or +0x90, w=42, h=30)
    const int targetRightX = (screenW / 2) + 80;     // 1040 at 1920p
    const int targetRightY = targetBaseY + 99;        // 1045 at 1080p
    ...
    __try {
        ...
        *reinterpret_cast<DWORD*>(reinterpret_cast<uintptr_t>(pBase) + 0x28) |= 0x11;
        ...
        // 3. Left wing: Cyan Operative button (+0x94 / +0x98 / +0x9C) [targetLeftX = 838, targetLeftY = 1045]
        void* leftWidgets[] = {
            *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(pCtrl27) + 0x94),
            *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(pCtrl27) + 0x98),
            *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(pCtrl27) + 0x9C)
        };
        for (void* pBtnLeft : leftWidgets) {
        ...
        void* rightWidgets[] = {
            *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(pCtrl27) + 0x8C),
            *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(pCtrl27) + 0x90)
        };
        for (void* pBtnRight : rightWidgets) {
...
static void RepositionCombatTactics(uintptr_t clientBase, void* pUI, int screenW, int screenH) {
    ...
    __try {
        ...
        if (pRootWidget && IsValidWidget(clientBase, pRootWidget)) {
            ...
            *reinterpret_cast<DWORD*>(reinterpret_cast<uintptr_t>(pRootWidget) + 0x28) &= ~0x00000001;
            *reinterpret_cast<DWORD*>(reinterpret_cast<uintptr_t>(pRootWidget) + 0x28) |= 0x00000010;
        }

        // Layout container handling for CViewInterlock
        uintptr_t pInterlockLayout = *reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(pInterlock) + 0x50);
        void* pInterlockLayoutRoot = nullptr;
        if (pInterlockLayout && !IsBadReadPtr(reinterpret_cast<void*>(pInterlockLayout), 0x30)) {
            pInterlockLayoutRoot = *reinterpret_cast<void**>(pInterlockLayout + 0x24);
            if (pInterlockLayoutRoot && IsValidWidget(clientBase, pInterlockLayoutRoot)) {
                *reinterpret_cast<DWORD*>(reinterpret_cast<uintptr_t>(pInterlockLayoutRoot) + 0x28) &= ~0x00000001;
                *reinterpret_cast<DWORD*>(reinterpret_cast<uintptr_t>(pInterlockLayoutRoot) + 0x28) |= 0x00000010;
            }
        }

        // Retrieve and persist the Combat Posture stance buttons:
        // 1. Focus / Stance Free (+0x6C, Interlock_Speed_Button - Blue/Cyan icon)
        static void* s_savedBtnFocus = nullptr;
        static void* s_savedBtnPower = nullptr;
        static void* s_savedBtnAttack = nullptr;
        static void* s_savedBtnDefense = nullptr;

        void* pBtnFocus = *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(pInterlock) + 0x6C);
        if (pBtnFocus && IsValidWidget(clientBase, pBtnFocus)) s_savedBtnFocus = pBtnFocus;
        else if (s_savedBtnFocus && IsValidWidget(clientBase, s_savedBtnFocus)) pBtnFocus = s_savedBtnFocus;

        // 2. Power (+0x70, Interlock_Power_Button - Red fist icon)
        void* pBtnPower = *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(pInterlock) + 0x70);
        if (pBtnPower && IsValidWidget(clientBase, pBtnPower)) s_savedBtnPower = pBtnPower;
        else if (s_savedBtnPower && IsValidWidget(clientBase, s_savedBtnPower)) pBtnPower = s_savedBtnPower;

        // 3. Attack / Grab (+0x74, Interlock_Grab_Button - Green claw icon)
        void* pBtnAttack = *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(pInterlock) + 0x74);
        if (pBtnAttack && IsValidWidget(clientBase, pBtnAttack)) s_savedBtnAttack = pBtnAttack;
        else if (s_savedBtnAttack && IsValidWidget(clientBase, s_savedBtnAttack)) pBtnAttack = s_savedBtnAttack;

        // 4. Defense / Block (+0x80, Interlock_Block_Button / +0x7C Withdraw - Yellow shield/hand icon)
        void* pBtnDefense = *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(pInterlock) + 0x80);
        if (!pBtnDefense || !IsValidWidget(clientBase, pBtnDefense)) {
            pBtnDefense = *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(pInterlock) + 0x7C);
        }
        if (pBtnDefense && IsValidWidget(clientBase, pBtnDefense)) s_savedBtnDefense = pBtnDefense;
        else if (s_savedBtnDefense && IsValidWidget(clientBase, s_savedBtnDefense)) pBtnDefense = s_savedBtnDefense;

        static int s_tacticsLogCount = 0;
        if (s_tacticsLogCount++ < 3) {
            Log("[mxohax] RepositionCombatTactics: pInterlock=0x%p, pRootWidget=0x%p, focus=0x%p, power=0x%p, grab=0x%p, block=0x%p\n",
                pInterlock, pRootWidget, pBtnFocus, pBtnPower, pBtnAttack, pBtnDefense);
        }
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (pBtn && IsValidWidget(clientBase, pBtn)) {
                ...
                pSetPosition(pBtn, posX, btnY, nullptr, 1);
        ...
        for (uintptr_t off = 0x04; off <= 0x350; off += 4) {
            if (off == 0x50 || off == 0x6C || off == 0x70 || off == 0x74 || off == 0x7C || off == 0x80) continue; // Keep layout and 4 stance buttons!
            void* pW = *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(pInterlock) + off);
            if (pW && pW != pBtnFocus && pW != pBtnPower && pW != pBtnAttack && pW != pBtnDefense &&
                pW != pRootWidget && pW != pInterlock && pW != reinterpret_cast<void*>(pInterlockLayout) &&
                pW != pInterlockLayoutRoot && IsValidWidget(clientBase, pW)) {
                *reinterpret_cast<DWORD*>(reinterpret_cast<uintptr_t>(pW) + 0x28) &= ~0x00000011;
...
static void __fastcall DetourFrameTick(void* pThis, void* /*edx*/) {
    ...
    if (s_inWorldSticky) {
        ...
        *reinterpret_cast<BYTE*>(clientBase + 0x008AACC8) = 0;
        *reinterpret_cast<BYTE*>(clientBase + 0x008AACC9) = 0;
        *reinterpret_cast<BYTE*>(clientBase + 0x0084A02C) = 0; // Lens flare active flag
        *reinterpret_cast<float*>(clientBase + 0x008E357C) = 0.0f;

        // Tame Blinding "Nuclear Glare" & Auto-Inflating Bloom (Restore Launch-Era 2005 Matrix Noir Atmosphere)
        // Verified exact CVar object addresses in client.dll:
        SetCVarFloat(clientBase + 0x008A9B44, 0.0f); // FX_Light_Glare_Alpha (eliminates blinding solar blowout)
        SetCVarFloat(clientBase + 0x008A9C18, 0.0f); // FX_Light_Glare_HalfSize (eliminates blinding solar disk)
        SetCVarFloat(clientBase + 0x008A9BBC, 0.0f); // FX_Light_Glare_Dst_0
        SetCVarFloat(clientBase + 0x008A9C54, 0.0f); // FX_Light_Glare_Dst_1
        SetCVarFloat(clientBase + 0x00909694, 0.0f); // WR_LensFlare_Scale (zero lens flare blinding)
        SetCVarFloat(clientBase + 0x009096C8, 0.0f); // WR_LensFlare_InOut
        SetCVarFloat(clientBase + 0x009096FC, 0.0f); // WR_LensFlare_Y_Limit
        SetCVarFloat(clientBase + 0x0090962C, 0.0f); // WR_SkyBox_Base_Sprite_Size (zero sun sprite size)
        SetCVarFloat(clientBase + 0x008A2FAC, 0.0f); // ScreenFilters_Enable (disable bloom / glow screen filters)
        SetCVarFloat(clientBase + 0x008A3390, 0.0f); // ScreenFilters_Screen_Glow_BrightFactor (zero bloom blowout)
        SetCVarFloat(clientBase + 0x008A33C4, 0.0f); // ScreenFilters_Screen_Glow_AutoSetVals (disable dynamic bloom blowouts)
        SetCVarFloat(clientBase + 0x008A350C, 0.0f); // ScreenFilters_Screen_Glow_Trg_OutDoors_Day_BrightFactor (tames outdoor sky glare)
        SetCVarFloat(clientBase + 0x008A335C, 0.0f); // ScreenFilters_Screen_Glow_BlurScale (tame blurry washout)
        SetCVarFloat(clientBase + 0x008A34D8, 0.0f); // ScreenFilters_Screen_Glow_Trg_OutDoors_Day_BlurScale
        SetCVarFloat(clientBase + 0x008A3328, 1.0f); // ScreenFilters_Screen_Glow_SubVal (prevents background sky bloom blowout)
        SetCVarFloat(clientBase + 0x008A34A4, 1.0f); // ScreenFilters_Screen_Glow_Trg_OutDoors_Day_SubVal
        SetCVarFloat(clientBase + 0x00908C90, (float)g_playerY); // DL_FloorHeight (track player elevation)
    ...
    if (s_tickCount % 50 == 0) {
        ...
        int curMarginState = GetMarginStateId();
        BYTE realCount = pMarginMgr ? *reinterpret_cast<BYTE*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x640) : 0;
        Log("[mxohax] DetourFrameTick: Tick #%d active (State=%u, MarginState=%d, realCount=%u, 5DActive=%d, renderFlag=%d, inWorld=%u)\n",
            s_tickCount, curState, curMarginState, realCount, s_screen5DActive ? 1 : 0, curRenderFlag, inWorld);
    }
    ...
    if (pWorldMgr && !s_inWorldSticky) {
        ...
        if (pState) {
            if (*pState == 0 || *pState == 1) {
                ...
                if (pMarginMgr) {
                    int curMarginState = GetMarginStateId();
                    if (curMarginState == 0 && s_state1Ticks >= 10) {
                        typedef void (__thiscall *TransitionToState_t)(void* pMgr, DWORD newStateId);
                        TransitionToState_t Transition = reinterpret_cast<TransitionToState_t>(0x00428FF0);
                        Transition(pMarginMgr, 4);
                        Log("[mxohax] DetourFrameTick: Advanced pMarginMgr from State 0 to State 4!\n");
                    }
                }
                if (s_state1Ticks >= 20 && !s_autoJackInDone && !s_worldTransitionDone && !s_charCreationSubmitted) {
                    if (g_AutoJackInRequested || g_CommandLineCharSpecified) {
                        Log("[mxohax] DetourFrameTick: State %u AutoJackIn threshold reached (tick %d) -> triggering AutoJackIn...\n", *pState, s_state1Ticks);
    ...
    int curMarginState = GetMarginStateId();
    DWORD curWorldState = pWorldMgr ? *reinterpret_cast<DWORD*>(reinterpret_cast<DWORD>(pWorldMgr) + 0x1C) : 0;

    // TRIGGER 1: Character Creation completed / Finish clicked ('CHE1')
    if (s_charCreationSubmitted && !s_worldTransitionDone) {
        s_charCreationTicks++;
        void* pMarginMgr = *reinterpret_cast<void**>(0x004B3A44);
        uint32_t assignedCharId = 0;
        const char* pMarginHandle = nullptr;
        BYTE realCount = 0;

        if (pMarginMgr) {
            assignedCharId = *reinterpret_cast<uint32_t*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x524);
            realCount = *reinterpret_cast<BYTE*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x640);
            pMarginHandle = reinterpret_cast<const char*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x528);
            if (!pMarginHandle || pMarginHandle[0] == '\0') {
                pMarginHandle = reinterpret_cast<const char*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0xFC);
            }
        }

        bool charReady = false;
        if (assignedCharId > 0) {
            g_ActiveCharId = assignedCharId;
            charReady = true;
            Log("[mxohax] Char Creation: Server assigned charId=%u from pMarginMgr+0x524!\n", g_ActiveCharId);
        } else if (realCount > 0 && pMarginMgr) {
            uint8_t selIdx = *reinterpret_cast<uint8_t*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x66C);
            if (selIdx >= 5) selIdx = 0;
            uintptr_t pCharEntry = *reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x644 + selIdx * 4);
            if (pCharEntry && pCharEntry != reinterpret_cast<uintptr_t>(&g_SyntheticCharObj)) {
                uintptr_t pCharRec = *reinterpret_cast<uintptr_t*>(pCharEntry + 0x10);
                if (pCharRec) {
                    uint32_t cId = *reinterpret_cast<uint32_t*>(pCharRec + 3);
                    if (cId > 0) {
                        g_ActiveCharId = cId;
                        charReady = true;
                        Log("[mxohax] Char Creation: Found charId=%u in MarginMgr entry #%d!\n", g_ActiveCharId, selIdx);
                    }
                }
            }
        }
        ...
        // Transition once character is confirmed by MarginServer, OR Margin reached confirmation state (10/13/9), OR debounce timeout (>= 40 ticks = ~0.6s)
        if (charReady || (curMarginState == 10 || curMarginState == 13 || curMarginState == 9) || s_charCreationTicks >= 40) {
            if (g_ActiveCharId == 0) {
                if (assignedCharId > 0) g_ActiveCharId = assignedCharId;
                else g_ActiveCharId = 360; // Fallback operative ID
            }
            if (g_ActiveCharName[0] == '\0') {
                strncpy_s(g_ActiveCharName, sizeof(g_ActiveCharName), "Operative", _TRUNCATE);
            }

            Log("[mxohax] Char Creation: Entering world for '%s' (charId=%u, marginState=%d, ticks=%d)...\n",
                g_ActiveCharName, g_ActiveCharId, curMarginState, s_charCreationTicks);
            ...
            s_screen5DActive = false;
            s_screen5DEverOpened = false;
            s_charCreationSubmitted = false;

            if (pMarginMgr && curMarginState != 9) {
                typedef void (__thiscall *TransitionToState_t)(void* pMgr, DWORD newStateId);
                TransitionToState_t Transition = reinterpret_cast<TransitionToState_t>(0x00428FF0);
                Transition(pMarginMgr, 9);
            }
    ...
    // TRIGGER 2: Existing Character handling when Margin reaches State 4 or State 0
    if (!s_worldTransitionDone && !s_charCreationSubmitted && curWorldState != 3 && curWorldState != 4) {
        if (curMarginState == 4 || curMarginState == 0) {
            s_marginState4Ticks++;
            void* pMarginMgr = *reinterpret_cast<void**>(0x004B3A44);
            if (pMarginMgr) {
                BYTE realCount = *reinterpret_cast<BYTE*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x640);
                ...
                if (curMarginState == 0 && s_marginState4Ticks >= 10) {
                    typedef void (__thiscall *TransitionToState_t)(void* pMgr, DWORD newStateId);
                    TransitionToState_t Transition = reinterpret_cast<TransitionToState_t>(0x00428FF0);
                    Transition(pMarginMgr, 4);
                    curMarginState = GetMarginStateId();
                ...
                // Account has character(s) (either real server character or mounted synthetic operative):
                if (realCount > 0 && pChar0 != 0) {
                    // 1. Screen 0x5D was opened prematurely at startup before Margin authentication. Dismiss it!
                    void* pUI = GetCUIPointer(clientBase);
                    if (pUI && s_screen5DActive) {
                        SafeSetControlVisible(clientBase, pUI, 0x5D, false);
                        SafeHideControl(clientBase, pUI, 0x5D);
                        s_screen5DActive = false;
                        s_screen5DEverOpened = false;
                        Log("[mxohax] Existing characters detected (count=%u): Dismissed premature Screen 0x5D!\n", realCount);
                    }

                    // 2. Resolve target character index
                    int targetIdx = 0;
                    if (g_CommandLineCharSpecified && g_ActiveCharName[0] != '\0' && pChar0 != reinterpret_cast<uintptr_t>(&g_SyntheticCharObj)) {
                        for (int i = 0; i < realCount && i < 5; ++i) {
                            uintptr_t pEntry = *reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x644 + i * 4);
                            if (pEntry && pEntry != reinterpret_cast<uintptr_t>(&g_SyntheticCharObj)) {
                                uintptr_t pData = *reinterpret_cast<uintptr_t*>(pEntry + 0x14);
                                if (pData) {
                                    const char* handle = reinterpret_cast<const char*>(pData + 3);
                                    if (handle && _stricmp(handle, g_ActiveCharName) == 0) {
                                        targetIdx = i;
                                        break;
                                    }
                                }
                    ...
                    // Extract chosen character handle and charId if real character
                    if (pChar0 != reinterpret_cast<uintptr_t>(&g_SyntheticCharObj)) {
                        uintptr_t pChosenEntry = *reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(pMarginMgr) + 0x644 + targetIdx * 4);
                        if (pChosenEntry && pChosenEntry != reinterpret_cast<uintptr_t>(&g_SyntheticCharObj)) {
                            uintptr_t pCharData = *reinterpret_cast<uintptr_t*>(pChosenEntry + 0x14);
                            if (pCharData) {
                                const char* handle = reinterpret_cast<const char*>(pCharData + 3);
                                if (handle && handle[0] != '\0' && strlen(handle) < 32) {
                                    strncpy_s(g_ActiveCharName, sizeof(g_ActiveCharName), handle, _TRUNCATE);
                            ...
                            uintptr_t pCharRec = *reinterpret_cast<uintptr_t*>(pChosenEntry + 0x10);
                            if (pCharRec) {
                                uint32_t cId = *reinterpret_cast<uint32_t*>(pCharRec + 3);
                                if (cId > 0) g_ActiveCharId = cId;
                            }
                    ...
                    // New account, no characters made yet.
                    if (g_AutoJackInRequested || g_CommandLineCharSpecified) {
                        if (s_marginState4Ticks >= 15) {
                            DWORD pThisDword = reinterpret_cast<DWORD>(pMarginMgr);
                            SetupSyntheticCharManager(pThisDword);
                            Log("[mxohax] AutoJackIn (no chars): Using synthetic operative '%s' (charId=%u)...\n",
                                g_ActiveCharName, g_ActiveCharId);
                            if (TryAutoJackIn(clientBase)) {
                                s_worldTransitionDone = true;
                                s_autoJackInDone = true;
                        ...
                        // Interactive user with no characters -> ensure Screen 0x5D is visible for character creation
                        void* pUI = GetCUIPointer(clientBase);
                        if (pUI && !s_screen5DActive) {
                            SafeSetControlVisible(clientBase, pUI, 0x5D, true);
...
void WINAPI DetourExitProcess(UINT uExitCode) {
    ...
    Log("[mxohax] ExitProcess(%u / 0x%08X) called! ReturnAddress: 0x%p, WorldState=%u, inWorld=%d, streaming=%d (ThreadId=%u)\n",
        uExitCode, uExitCode, caller, curWorldState, s_inWorldSticky ? 1 : 0, s_inStreamingState4 ? 1 : 0, GetCurrentThreadId());

    if (s_inWorldSticky || s_inStreamingState4 || curWorldState >= 2 || uExitCode == 3221225477 || uExitCode >= 0x80000000) {
        Log("[mxohax] DetourExitProcess: SUPPRESSED exit call (staying alive in world/transition)!\n");
        return;
    }
...
void WINAPI DetourPostQuitMessage(int nExitCode) {
    ...
    Log("[mxohax] PostQuitMessage(%d) called! ReturnAddress: 0x%p, WorldState=%u, inWorld=%d, streaming=%d (ThreadId=%u)\n",
        nExitCode, caller, curWorldState, s_inWorldSticky ? 1 : 0, s_inStreamingState4 ? 1 : 0, GetCurrentThreadId());

    if (s_inWorldSticky || s_inStreamingState4 || curWorldState >= 2 || nExitCode != 0) {
        Log("[mxohax] DetourPostQuitMessage: SUPPRESSED exit loop call (staying alive in world/transition)!\n");
        return;
    }
...
static void __fastcall Safe_Interlock_Block_Button(void* pThis, void* /*edx*/) {
    ...
    if (!pBtn || IsBadReadPtr(pBtn, sizeof(void*))) {
        Log("[mxohax] Safe_Interlock_Block_Button: button ptr (+0x80/+0x7C) is null/invalid\n");
        return;
    }
    void* vtbl = *reinterpret_cast<void**>(pBtn);
    if (!vtbl || IsBadReadPtr(vtbl, 0xB0)) {
...
static void ApplyClientPatches(HMODULE hClient) {
    ...
    // Hook client.dll allocator refill at 0x00001B50
    ...
    if (MH_CreateHook(pRefill, &DetourRefill, reinterpret_cast<LPVOID*>(&OriginalRefill)) == MH_OK) {
        ...
        Log("[mxohax] SUCCESS: client.dll allocator refill hooked at 0x%p!\n", pRefill);
    }

    // Hook client.dll pool free at 0x00001BC0
    ...
    if (MH_CreateHook(pPoolFree, &DetourPoolFree, reinterpret_cast<LPVOID*>(&OriginalPoolFree)) == MH_OK) {
        ...
        Log("[mxohax] SUCCESS: client.dll pool free hooked at 0x%p!\n", pPoolFree);
    }
    ...
    if (MH_CreateHook(pFrameTick, &DetourFrameTick, reinterpret_cast<LPVOID*>(&OriginalFrameTick)) == MH_OK) {
        ...
        // 3. Hook SetControlVisible at 0x0001DB80
    ...
    // 3a. Hook client.dll + 0x0002AC10 (Play selected character from Screen 0x4B)
    ...
    if (MH_CreateHook(pClientPlay, &DetourClientPlayChar, reinterpret_cast<LPVOID*>(&OriginalClientPlayChar)) == MH_OK) {
        ...
        Log("[mxohax] SUCCESS: client.dll ClientPlayChar (0x1002AC10) hooked at 0x%p!\n", pClientPlay);
    }

    // 3a1. Hook client.dll + 0x000C2120 (SelectCharacterInList)
    ...
    if (MH_CreateHook(pSelectInList, &DetourSelectCharacterInList, reinterpret_cast<LPVOID*>(&OriginalSelectCharacterInList)) == MH_OK) {
        ...
        Log("[mxohax] SUCCESS: client.dll SelectCharacterInList (0x100C2120) hooked at 0x%p!\n", pSelectInList);
    }

    // 3a2. Hook client.dll + 0x0004AE30 (CLARSICharCreateView::HandleMessage)
    ...
    if (MH_CreateHook(pCharCreateMsg, &DetourCharCreateHandleMessage, reinterpret_cast<LPVOID*>(&OriginalCharCreateHandleMessage)) == MH_OK) {
        ...
        Log("[mxohax] SUCCESS: client.dll CLARSICharCreateView::HandleMessage (0x1004AE30) hooked at 0x%p!\n", pCharCreateMsg);
    }
    ...
    *reinterpret_cast<const char**>(clientBase + 0x00896E4C) = s_initMetrPath;
    ...
    // Patch A: Fix native client.dll + 0x0012196E bug where 1 character forces Screen 0x5D (Char Create)
    // Native: movzx esi, bl (3); call [eax+0xc8] (6); cmp esi, eax (2); jb 0x101219b9 (2) -> 13 bytes
    // Patched: xor esi, esi (2); call [eax+0xc8] (6); test eax, eax (2); jnz 0x101219b9 (2); nop (1) -> 13 bytes
    ...
    if (VirtualProtect(pCharSelectPatch, 13, PAGE_EXECUTE_READWRITE, &oldProtA)) {
        static const BYTE patchA[13] = {
            ...
            0xFF, 0x90, 0xC8, 0x00, 0x00, 0x00, // call dword ptr [eax + 0xC8] (GetCharacterCount)
            0x85, 0xC0,                         // test eax, eax
            0x75, 0x3F,                         // jnz +0x3F -> clientBase + 0x001219B9 (Screen 0x4B Char Selection)
            0x90                                // nop (falls through to 0x0012197B Screen 0x5D Char Create if 0 chars)
        };
        ...
        Log("[mxohax] SUCCESS: Patched client.dll + 0x0012196E (13 bytes) for authentic character select vs create routing!\n");
    }
    ...
    // Patch K: 0x0012417B: 8 bytes safe bypass of null [0x108abb90] camera deref in EnterWorldWithCharacter
    // Original: 85 FF (test edi, edi); 0F 84 E7 00 00 00 (je 0x1012426A)
    // Patched:  E9 EA 00 00 00 (jmp 0x1012426A); 90 90 90 (nop * 3)
    LPVOID pEnterWorldCamBypass = reinterpret_cast<LPVOID>(clientBase + 0x0012417B);
    if (VirtualProtect(pEnterWorldCamBypass, 8, PAGE_EXECUTE_READWRITE, &oldProt)) {
        static const BYTE patchK[8] = { 0xE9, 0xEA, 0x00, 0x00, 0x00, 0x90, 0x90, 0x90 };
        memcpy(pEnterWorldCamBypass, patchK, 8);
        VirtualProtect(pEnterWorldCamBypass, 8, oldProt, &oldProt);
        FlushInstructionCache(GetCurrentProcess(), pEnterWorldCamBypass, 8);
        Log("[mxohax] SUCCESS: Patched client.dll + 0x0012417B (8 bytes) to safely bypass null player deref in EnterWorldWithCharacter!\n");
    }
    ...
    // Permanently neutralize lens flare and solar blowout / nuclear light glare
    LPVOID pLensFlareRender = reinterpret_cast<LPVOID>(clientBase + 0x0014A250);
    if (VirtualProtect(pLensFlareRender, 3, PAGE_EXECUTE_READWRITE, &oldProt)) {
        BYTE ret4[3] = { 0xC2, 0x04, 0x00 }; // ret 4
        memcpy(pLensFlareRender, ret4, 3);
        VirtualProtect(pLensFlareRender, 3, oldProt, &oldProt);
        FlushInstructionCache(GetCurrentProcess(), pLensFlareRender, 3);
        Log("[mxohax] SUCCESS: Patched client.dll + 0x0014A250 (ret 4) to permanently neutralize CLensFlare::Render!\n");
    }

    LPVOID pLensFlareHasFlare = reinterpret_cast<LPVOID>(clientBase + 0x0014A0A0);
    if (VirtualProtect(pLensFlareHasFlare, 3, PAGE_EXECUTE_READWRITE, &oldProt)) {
        BYTE xorRet[3] = { 0x31, 0xC0, 0xC3 }; // xor eax, eax; ret
        memcpy(pLensFlareHasFlare, xorRet, 3);
        VirtualProtect(pLensFlareHasFlare, 3, oldProt, &oldProt);
        FlushInstructionCache(GetCurrentProcess(), pLensFlareHasFlare, 3);
        Log("[mxohax] SUCCESS: Patched client.dll + 0x0014A0A0 (xor eax, eax; ret) to bypass lens flare rendering!\n");
    }

    LPVOID pLightGlareRender = reinterpret_cast<LPVOID>(clientBase + 0x002412B0);
    if (VirtualProtect(pLightGlareRender, 3, PAGE_EXECUTE_READWRITE, &oldProt)) {
        BYTE ret4[3] = { 0xC2, 0x04, 0x00 }; // ret 4
        memcpy(pLightGlareRender, ret4, 3);
        VirtualProtect(pLightGlareRender, 3, oldProt, &oldProt);
        FlushInstructionCache(GetCurrentProcess(), pLightGlareRender, 3);
        Log("[mxohax] SUCCESS: Patched client.dll + 0x002412B0 (ret 4) to permanently neutralize CLightGlareFX::Render!\n");
    }

    LPVOID pSkySprite1 = reinterpret_cast<LPVOID>(clientBase + 0x003F1410);
    if (VirtualProtect(pSkySprite1, 3, PAGE_EXECUTE_READWRITE, &oldProt)) {
        BYTE retC[3] = { 0xC2, 0x0C, 0x00 }; // ret 0xC
        memcpy(pSkySprite1, retC, 3);
        VirtualProtect(pSkySprite1, 3, oldProt, &oldProt);
        FlushInstructionCache(GetCurrentProcess(), pSkySprite1, 3);
        Log("[mxohax] SUCCESS: Patched client.dll + 0x003F1410 (ret 0xC) to neutralize sky sun sprites!\n");
    }

    LPVOID pSkySprite2 = reinterpret_cast<LPVOID>(clientBase + 0x003F1250);
    if (VirtualProtect(pSkySprite2, 3, PAGE_EXECUTE_READWRITE, &oldProt)) {
        BYTE retC[3] = { 0xC2, 0x0C, 0x00 }; // ret 0xC
        memcpy(pSkySprite2, retC, 3);
        VirtualProtect(pSkySprite2, 3, oldProt, &oldProt);
        FlushInstructionCache(GetCurrentProcess(), pSkySprite2, 3);
        Log("[mxohax] SUCCESS: Patched client.dll + 0x003F1250 (ret 0xC) to neutralize sky sun corona!\n");
    }

    LPVOID pSkyBoxLensFlare = reinterpret_cast<LPVOID>(clientBase + 0x003F21D0);
    if (VirtualProtect(pSkyBoxLensFlare, 1, PAGE_EXECUTE_READWRITE, &oldProt)) {
        BYTE retInst = 0xC3; // ret
        memcpy(pSkyBoxLensFlare, &retInst, 1);
        VirtualProtect(pSkyBoxLensFlare, 1, oldProt, &oldProt);
        FlushInstructionCache(GetCurrentProcess(), pSkyBoxLensFlare, 1);
        Log("[mxohax] SUCCESS: Patched client.dll + 0x003F21D0 (ret) to permanently neutralize SkyBox::RenderLensFlare!\n");
    }

    LPVOID pSkyBoxLensFlareImpl = reinterpret_cast<LPVOID>(clientBase + 0x003F1DA0);
    if (VirtualProtect(pSkyBoxLensFlareImpl, 3, PAGE_EXECUTE_READWRITE, &oldProt)) {
        BYTE ret8[3] = { 0xC2, 0x08, 0x00 }; // ret 8
        memcpy(pSkyBoxLensFlareImpl, ret8, 3);
        VirtualProtect(pSkyBoxLensFlareImpl, 3, oldProt, &oldProt);
        FlushInstructionCache(GetCurrentProcess(), pSkyBoxLensFlareImpl, 3);
        Log("[mxohax] SUCCESS: Patched client.dll + 0x003F1DA0 (ret 8) to neutralize SkyBox lens flare geometry builder!\n");
    }

    LPVOID pRenderAllLightGlares = reinterpret_cast<LPVOID>(clientBase + 0x000E1140);
    if (VirtualProtect(pRenderAllLightGlares, 1, PAGE_EXECUTE_READWRITE, &oldProt)) {
        BYTE retInst = 0xC3; // ret
        memcpy(pRenderAllLightGlares, &retInst, 1);
        VirtualProtect(pRenderAllLightGlares, 1, oldProt, &oldProt);
        FlushInstructionCache(GetCurrentProcess(), pRenderAllLightGlares, 1);
        Log("[mxohax] SUCCESS: Patched client.dll + 0x000E1140 (ret) to neutralize RenderAllLightGlares!\n");
    }

    LPVOID pScreenFXRender = reinterpret_cast<LPVOID>(clientBase + 0x00240351);
    if (VirtualProtect(pScreenFXRender, 6, PAGE_EXECUTE_READWRITE, &oldProt)) {
        BYTE jmpCleanup[6] = { 0xE9, 0x4D, 0x02, 0x00, 0x00, 0x90 }; // jmp 0x102405a3; nop
        memcpy(pScreenFXRender, jmpCleanup, 6);
        VirtualProtect(pScreenFXRender, 6, oldProt, &oldProt);
        FlushInstructionCache(GetCurrentProcess(), pScreenFXRender, 6);
        Log("[mxohax] SUCCESS: Patched client.dll + 0x00240351 (jmp) to safely bypass ScreenFX glare quad rendering!\n");
    }

    LPVOID pScreenFiltersRender = reinterpret_cast<LPVOID>(clientBase + 0x001A7130);
    if (VirtualProtect(pScreenFiltersRender, 5, PAGE_EXECUTE_READWRITE, &oldProt)) {
        BYTE xorRet4[5] = { 0x31, 0xC0, 0xC2, 0x04, 0x00 }; // xor eax, eax; ret 4
        memcpy(pScreenFiltersRender, xorRet4, 5);
        VirtualProtect(pScreenFiltersRender, 5, oldProt, &oldProt);
        FlushInstructionCache(GetCurrentProcess(), pScreenFiltersRender, 5);
        Log("[mxohax] SUCCESS: Patched client.dll + 0x001A7130 (xor eax, eax; ret 4) to neutralize ScreenFilters bloom rendering!\n");
    }
    ...
    if (VirtualProtect(pDaylightSkyPaneColor, 7, PAGE_EXECUTE_READWRITE, &oldProt)) {
        // C7 45 FC 14 22 14 00 -> mov dword ptr [ebp - 4], 0x00142214 (authentic Matrix noir green)
        ...
        Log("[mxohax] SUCCESS: Patched client.dll + 0x003F16A8 to Matrix noir green atmosphere (0x00142214)!\n");
    }
...
static void InitializeMxOHaxSynchronous() {
    ...
    if (MH_CreateHook(pMarginSelect, &DetourMarginSelectChar, reinterpret_cast<LPVOID*>(&OriginalMarginSelectChar)) == MH_OK) {
        ...
        Log("[mxohax] SUCCESS: matrix.exe MarginMgr::SelectCharacter (0x00429F20) hooked at 0x%p!\n", pMarginSelect);
    }
